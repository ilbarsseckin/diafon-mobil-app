...
class CallScreen extends StatefulWidget {
  final String peerUserId;     // karşı tarafın id'si
  final String peerName;       // karşı tarafın adı
  final bool isCaller;         // arayan mı (true) yoksa alan mı (false)
  final String? incomingCallId; // gelen çağrıda callId
...
class _CallScreenState extends State<CallScreen> {
  ...
  final Map<String, dynamic> _iceConfig = {
    'iceServers': [
      ...
      // Coturn (kendi TURN sunucumuz) — ileride TLS ile
      {
  ...
  Future<void> _init() async {
    ...
    // ICE adayı bulununca karşıya gönder
    ...
    if (widget.isCaller) {
      ...
      // Alan taraf: çağrıyı kabul et, offer bekle
  ...
  void _setupSocketListeners() {
    // Çağrı kabul edildi (arayan için) -> offer gönder
    SocketService.on('call:accepted', (data) async {
      _callId = data['callId'];
      await _createOffer();
    });

    // Offer geldi (alan için) -> answer oluştur
